...
int main(void) {
    ...
    while (fgets(prompt, sizeof(prompt), stdin)) {
        ...
        // 기존 test처럼 context 유지 + 전체 응답 반환
        int ret = ai_chat_with_context(
            context_mem,       // mmap된 context buffer
            &cm_size,          // 현재 context size
            prompt,            // 사용자 입력
            response,          // AI 전체 응답
            sizeof(response),
            MODEL_NAME_QWEN3
...
static int call_chat_api(cJSON *messages, const char *model, char *out, size_t out_sz) {
    ...
    // stream은 boolean 타입이어야 함(false로 설정)
